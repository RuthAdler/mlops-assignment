...
def eval_one(question: dict, agent_url: str, tags: dict[str, str] | None = None) -> dict:
    """Score one question. Per-iteration correctness is re-checked by executing
    each SQL the agent emitted against the target DB and comparing to gold rows.
    """
    q = question["question"]
    db = question["db_id"]
    ...
    for sql in attempts:
        ...
        per_iteration.append({"sql": sql, "ok": ok, "error": err, "correct": correct})
    ...
    return {
        ...
        "n_iterations": body.get("iterations", len(attempts)),
...
def summarize(results: list[dict]) -> dict:
    ...
    mean_iter = sum(r.get("n_iterations", 0) for r in results) / n
